...
class TestWord(unittest.TestCase):
    ...
    def test_trim(self):
        self.assertEqual(self.words.trim([' .,test$&', 'another1^#@test ']), ['test', 'another1test'])
    ...
    def test_largest_word_non_empty_file(self):
        ...
        result_validation = test_word_support.get_largest_word(f'{TestWord.FILE_DIR}words.txt')
    ...
    def test_largest_word_one_word_file(self):
        ...
        result_validation = test_word_support.get_largest_word(f'{TestWord.FILE_DIR}words2.txt')
        self.assertEqual(result, result_validation)

    def test_largest_word_empty_file(self):
        result = self.empty_file.process_largest_word()
        result_validation = test_word_support.get_largest_word(f'{TestWord.FILE_DIR}words3.txt')
        self.assertEqual(result, result_validation)

    def test_largest_word_no_words_file(self):
        result = self.no_words_file.process_largest_word()
        result_validation = test_word_support.get_largest_word(f'{TestWord.FILE_DIR}words4.txt')
        self.assertEqual(result, result_validation)
